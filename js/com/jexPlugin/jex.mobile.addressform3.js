	...
	var JexMobileAddressForm2 = JexPlugin.extend({
		...
		getAddressForm : function($jq) {
			...
			$.get('../../com/addressform.html').done(function(dat)	{
				...
				/**
				var acntAreaHtml = ""
					+ '<div class="dt txt15">' + adrsTitle + '</div>'
					+ '<div class="dd mt10 div_addr" id="' + adrsAreaId + '" >'
						+ '<div class="floatW">'
							+ '<input type="hidden" id="BSUN_RDND_DCD" value="2"><!-- 도로명주소구분코드 [1:지번(구주소), 2:도로명] -->'
							+ '<input type="text" id="BSUN_ZPCD" class="wid67 ZIPCODE param_key copy_key" title="우편번호 입력" readonly="readonly"'
									+ 'data-valid-check=\'{"required":"본사/사업장 주소를입력해주세요."}\' data-id="MNOF_RDNM_ZPCD">'
							+ '<input type="hidden" class="BLDN_MNGM_NO copy_key" data-id="MOMO_NO" id="BSUN_BLDN_MNGM_NO">'
							+ '<span class="btn_r_wrap"><button id="addr_find" type="button" class="btn_gray2" data-step="2400">우편번호 찾기</button></span>'
						+ '</div>'
						+ '<div class="floatW pt5"><input type="text" id="BSUN_BSC_ADR" class="ADDR param_key copy_key" title="주소 입력" readonly="readonly"'
								+ 'data-valid-check=\'{"required":"본사/사업장 주소를 입력해주세요."}\' data-id="MNOF_RDNM_BSC_ADR"></div>'
						+ '<div class="floatW pt5"><input type="text" id="BSUN_DTL_ADR" class="ADDR_DTL param_key copy_key" title="상세주소 입력" readonly="readonly"'
								+ 'data-valid-check=\'{"required":"본사/사업장 주소를 입력해주세요."}\' data-id="MNOF_RDNM_DTL_ADR"></div>'
					+ '</div>';
				acntAreaHtml = $(acntAreaHtml);
				
				acntAreaHtml.find("#addr_find").on('click', function(evt) { //우편번호 찾기 버튼 Click Event
					var $this = $(this);
					var target_id = $(this).attr('id');
					
					//step2400
					adrs_com.goNext($this.attr('data-step'), $this.closest('.div_addr'));
				});
...
//			$('body').append(fnt_lon_080112_1);
		...
		drawAdrsAreaHtml : function($jq) {
			...
			var acntAreaHtml = ""
				+ '<div class="comp_wrap address">'
				+ '	<div class="input has_search has_value">'
				+ '		<input type="hidden" id="BSUN_RDND_DCD" value="2"><!-- 도로명주소구분코드 [1:지번(구주소), 2:도로명] -->'
				+ '		<input type="text" id="BSUN_ZPCD" placeholder="우편번호" readonly="readonly"/>'
				+ '		<button type="button" class="search" id="addr_find" data-step="2400" data-jx-addressform2-lang="' + adrsLang + '">검색</button>'
				+ '		<button type="button" class="del" tabindex="0" aria-hidden="true">텍스트삭제</button>'
				+ '	</div>'
				+ '	<div class="input">'
				+ '		<input type="text" id="BSUN_BSC_ADR" placeholder="주소입력" class="ADDR param_key copy_key" data-valid-check=\'{"required":"본사/사업장 주소를 입력해주세요."}\' data-id="MNOF_RDNM_BSC_ADR" readonly="readonly"/>'
				+ '	</div>'
				+ '	<div class="input">'
				+ '		<input type="text" id="BSUN_DTL_ADR" placeholder="상세주소 입력" class="ADDR_DTL param_key copy_key"  data-valid-check=\'{"required":"본사/사업장 주소를 입력해주세요."}\' data-id="MNOF_RDNM_DTL_ADR" readonly="readonly"/>'
				+ '	</div>'
				+ '</div>'
...
//								+ 'data-valid-check=\'{"required":"본사/사업장 주소를입력해주세요."}\' data-id="MNOF_RDNM_ZPCD">'
//						+ '<input type="hidden" class="BLDN_MNGM_NO copy_key" data-id="MOMO_NO" id="BSUN_BLDN_MNGM_NO">'
//						+ '<span class="btn_r_wrap"><button id="addr_find" type="button" class="btn_gray2" data-step="2400" data-jx-addressform2-lang="' + adrsLang + '">우편번호 찾기</button></span>'
//					+ '</div>'
//					+ '<div class="floatW pt5"><input type="text" id="BSUN_BSC_ADR" class="ADDR param_key copy_key" title="주소 입력" readonly="readonly"'
//							+ 'data-valid-check=\'{"required":"본사/사업장 주소를 입력해주세요."}\' data-id="MNOF_RDNM_BSC_ADR"></div>'
//					+ '<div class="floatW pt5"><input type="text" id="BSUN_DTL_ADR" class="ADDR_DTL param_key copy_key" title="상세주소 입력" readonly="readonly"'
...
var step2400 = {
	...
	add_event: function() {
		...
		$('#btn_confirm_addr').off("click").on("click", function() {
			...
			// 사업자당행고객일 경우
			//if (target_id == 'div_search_company') {
			...
			if (msg) {
				return adrs_com.popup({
					...
					txt: msg,
					callback: function() {
						adrsForm.searchAddr.setAddr(self.target);
