var comJavascript_now = new Date();
...
Date.prototype.format = function (f) {
	...
	return f.replace(/(yyyy|yy|mm|dd|E|hh|HH|mi|ss|a\/p)/gi, function ($1) {
		switch($1) {
...
var comJavascript_getServerTime = comJavascript_now.getFullYear() + "-" + (comJavascript_now.getMonth() + 1).zf(2) + "-" + comJavascript_now.getDate().zf(2);
comJavascript_getServerTime += "-" + comJavascript_now.getHours().zf(2) + "-" + comJavascript_now.getMinutes().zf(2) + "-" + comJavascript_now.getSeconds().zf(2);
comJavascript_getServerTime += "-000";


var g_j_time = new Date();

<!-- 서버시간 가져오기 -->
function g_getDate (dat) {
	result = "";
	var _date = new Date();
	var tmp_date = new Date();
	var _term = (tmp_date.getTime() - g_j_time.getTime());
	try {
			var _sTime = comJavascript_getServerTime;
			var _dt = _sTime.split("-");
			_date.setFullYear(_dt[0]);
			_date.setMonth((Number(_dt[1]) - 1));
			_date.setDate(_dt[2]);
			_date.setHours(_dt[3]);
			_date.setMinutes(_dt[4]);
			_date.setSeconds(_dt[5]);
			_date.setTime(_date.getTime() + _term);
			
	} catch (e) {
			console.log("time set error : " + e);
			alert("g_getDate error" + e);
	}
	console.log("requestServerTime : " + _date.format(dat));
	return _date.format(dat);	
}

function g_serverTime () {
	var _date = new Date();
	var tmp_date = new Date();
	var _term = (tmp_date.getTime() - g_j_time.getTime());
	try {
...
function comUtil_showPuclibCertList(stepNo, divId, certType, clickFncNm, taxCertYn, listType) { 
	...
	$.get(formFilePath).done(function(dat){
		...
		if("C" == certType) {
			$("#cert_type_name").text("사업자용 공동인증서");
			$("#cert_reg_btn").show();
		} else if("P" == certType) {
			$("#cert_type_name").text("개인용 공동인증서");
			$("#cert_reg_btn").hide();
			
		} else {
			$("#cert_type_name").text("공동인증서");
		}
		
		//함수명 설정
		$("#cert_click_link").attr("data-jx-execute-target",clickFncNm);
		
		jex.setJexObj($("#" + certListDivNm));
		...
		if(_isRealApp) {
			...
			comWebkey_getDeviceLoginInfo(function() {
				...
				var jex_list = jex.getJexObj($("#com_pub_cert_list"), "JEX_MOBILE_LIST");
				jex_list.setAll(cert_list);
				
				if(cert_list.length == 0) {
					//인증서가 없음 화면이동
					$("#public_no_cert_list").show();
					$("#com_pub_cert_list").hide();
				} else {
					$("#public_no_cert_list").hide();
					$("#com_pub_cert_list").show();
					//jylee 작업할것
					comUtil_setCertStatusInfo();
				}
			});
		} else {
...
function comUtil_setCertStatusInfo() {
	$("#com_pub_cert_list > li").each(function () {
		let $liObj = $(this);
