...
(function() {
	...
	var colorSet	= {
	   ...
	   ,"onlyblue"  :  [ "#8cbdfa", "#8cbdfa", "#8cbdfa", "#8cbdfa", "#8cbdfa", "#8cbdfa" ]
	   ,"mgl" 		:  [ "#67dddd", "#67dddd", "#67dddd", "#67dddd", "#67dddd", "#67dddd" ] //myGradeLine
	   ,"purple" 	:  [ "#797ae6", "#68dcde" ,"#abacf0", "#676767", "#8cbdfa", "#fddc68" ] //Radar
	   ,"yellow" 	:  [ "#ffefbb", "#fba66d" ,"#676767"] //Radar
	   ,"white" 	:  [ "#ffffff", "#fec20a"] 
	   ...
	   ,"berrycoral" :  [ "#8cbdfa", "#fca66d" ] //berry+coral :: dashboard chart color
  }
	...
	var chartopt	= {
			emptypoint : function() {
				return {
					...
					,options : {pointBackgroundColor : "#fff"
								,pointRadius : 5
								,pointBorderWidth : 4
							}
				}
				...
				return {
					...
					scale:	{
						ticks: {	callback:	function(label, index, labels) { return ""; }
								,	beginAtZero: false  , min : 0, max : 120,  stepSize : 30 , fontSize : 14}
	...
	var chartlabel = {
			...
			,gradeMSG : function(element, dataset, index) {
				...
				var fontStyle	= 'normal';
				var fontFamily	= 'Helvetica Neue';
				ctx.fillStyle	= 'rgb(247, 140, 70, 1)';
				ctx.font = Chart.helpers.fontString(fontSize, fontStyle, fontFamily);
	...
	var chart = _attrM.extend({
		...
		,onload	 	: function()	{ 
			...
			var drawChart = function(ele) {				
				var chartData = {
						labels: ['', '', '', '' ],
						datasets: [	 { type: _this.type, label: '', backgroundColor: "#ffffff", data: [ 0, 0, 0, 0 ], borderColor: "rgb(94,178,251)", fill: true, borderWidth: 2 }
									,{ type: _this.type, label: '', borderColor: "#dddddd", backgroundColor: _this.colors[0,1,2,3,4,5], borderWidth: 2, fill: true, data: [ 0, 0, 0, 0 ] } ]
				};
